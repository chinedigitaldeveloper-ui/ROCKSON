...
- [ ] **P1-03: Deterministic Expression Evaluation** — Lower side-effecting multi-expression constructs through unique temporaries so source evaluation is deterministic left-to-right. Cover binary operands, call arguments, method receivers/arguments, constructor arguments, array receiver/index, and all other multi-expression lowering contexts emitted into C. Do not claim single-evaluation or left-to-right ordering is currently implemented; current codegen inherits C evaluation order and may evaluate array index expressions twice (H-08 / evaluation-order contract).
...
- [ ] **P1-05: Integer Arithmetic Safety** — Define and implement source-language integer arithmetic semantics that prevent ordinary accepted programs from invoking signed-C UB. Explicitly cover: signed addition overflow, signed subtraction overflow, signed multiplication overflow, unary negation overflow (`-INT_MIN`), division by zero, modulo by zero, `INT_MIN / -1`, and `INT_MIN % -1`. The final policy (trap / wrap / saturate) must be recorded in an ADR before implementation. (H-03)
...
- [ ] **P1-07: Symbol Safety — Full Generated Namespace** — Ensure accepted source identifiers cannot collide with the entire compiler-owned and runtime namespace. Contract: `RAW USER C SYMBOLS = NO`. Must cover at minimum: C keywords, C reserved identifiers, compiler-generated function names (`myc_gen_*`), compiler-generated constructor/helper names (`myc_gen_new_<Class>`, `myc_gen_init_<Class>`), compiler-generated method names, compiler-generated temporaries (`myc_tmp_*`), runtime helper names (`myc_rt_*`), hidden object fields (`vptr`, `base`), and any lambda/closure helper symbols. The compiler-owned namespace MUST use a prefix scheme that does not itself violate C reserved-identifier rules (e.g., `myc_rt_*`, `myc_tmp_*`, `myc_gen_*`). (H-07)
